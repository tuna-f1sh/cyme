...
Register-ArgumentCompleter -Native -CommandName 'cyme' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'cyme' {
            ...
            [CompletionResult]::new('--config-blocks', '--config-blocks', [CompletionResultType]::ParameterName, 'Specify the blocks which will be displayed for each configuration and in what order. Supply arg multiple times or csv to specify multiple blocks')
            [CompletionResult]::new('--interface-blocks', '--interface-blocks', [CompletionResultType]::ParameterName, 'Specify the blocks which will be displayed for each interface and in what order. Supply arg multiple times or csv to specify multiple blocks')
            [CompletionResult]::new('--endpoint-blocks', '--endpoint-blocks', [CompletionResultType]::ParameterName, 'Specify the blocks which will be displayed for each endpoint and in what order. Supply arg multiple times or csv to specify multiple blocks')
            ...
            [CompletionResult]::new('-v', '-v', [CompletionResultType]::ParameterName, 'Verbosity level (repeat provides count): 1 prints device configurations; 2 prints interfaces; 3 prints interface endpoints; 4 prints everything and more blocks')
            [CompletionResult]::new('--verbose', '--verbose', [CompletionResultType]::ParameterName, 'Verbosity level (repeat provides count): 1 prints device configurations; 2 prints interfaces; 3 prints interface endpoints; 4 prints everything and more blocks')
