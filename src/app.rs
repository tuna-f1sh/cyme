...
impl Blocks {
    ...
    pub fn format_device_value(&self, d: &system_profiler::USBDevice, pad: &PrintPadding) -> Option<String> {
        match self {
            ...
            Blocks::BranchPosition => Some(format!("{:3}", d.location_id.tree_positions.last().unwrap_or(&0))),
    ...
    fn format_bus_value(&self, bus: &system_profiler::USBBus, pad: &PrintPadding) -> Option<String> {
        match self {
            ...
            Blocks::HostController => Some(format!("{:pad$}", bus.host_controller, pad=pad.host_controller)),
            _ => None,
        }
    }
}
...
pub fn get_devices_padding_required(devices: &Vec<&system_profiler::USBDevice>) -> PrintPadding {
    ...
    let longest_serial = devices.iter().max_by_key(|x| x.serial_num.as_ref().unwrap_or(&String::new()).len());
    let longest_manufacturer = devices.iter().max_by_key(|x| x.manufacturer.as_ref().unwrap_or(&String::new()).len());
    let longest_tree = devices.iter().max_by_key(|x| x.location_id.tree_positions.len());

    PrintPadding {
        name: longest_name.map_or(0, |d| d.name.len()), 
        serial: longest_serial.map_or(0, |d| d.serial_num.as_ref().unwrap_or(&String::new()).len()), 
        manufacturer: longest_manufacturer.map_or(0, |d| d.manufacturer.as_ref().unwrap_or(&String::new()).len()), 
        tree_positions: longest_tree.map_or(0, |d| d.location_id.tree_positions.len() * 2), 
        ..Default::default() }
}

pub fn print_flattened_devices(devices: &Vec<&system_profiler::USBDevice>, db: &DeviceBlocks, settings: &PrintSettings) {
...
pub fn print_devices(devices: &Vec<system_profiler::USBDevice>, db: &DeviceBlocks, settings: &PrintSettings) {
    let pad: PrintPadding = if !settings.no_padding {
    ...
    for device in devices {
        print_device(device, db, &pad);
        device
            .devices.as_ref().map(|d| print_devices(d, db, settings));
    }
}

pub fn print_spdata(spdata: &system_profiler::SPUSBDataType, db: &DeviceBlocks, bb: &BusBlocks, settings: &PrintSettings) {
    let pad: PrintPadding = if !settings.no_padding {
